     ...
     function getStuffGiphy()
  {
    ...
    return (
        ...
        <button type="button" onClick={getStuffGiphy} ><i class="fas fa-search"></i></button>
            <input type="text" id="search_query" name="search_query" placeholder={annoyed ? "Bruh type something first":"Search stuff here"} onChange={(e) => Update('query',e.target.value)} />
        <select className="dropdown1" onChange={(e) => Update('mode',e.target.value) }>
        ...
        <div className="data">
        {
            Giphy.data && Giphy.data.map(data =><div className="card" key={data.id}><img src={data.images.original.url}/>
